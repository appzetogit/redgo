...
export default function Terms() {
  ...
  const [loading, setLoading] = useState(!window._termsCache)
  const [termsData, setTermsData] = useState(window._termsCache || {
  ...
  const fetchTermsData = async () => {
    try {
      ...
      if (response.data.success) {
        const data = response.data.data || { title: 'Terms of Service', content: '' }
        setTermsData(data)
        window._termsCache = data // Instant cache store
      }
    } catch (error) {
  ...
  return (
    <AnimatedPage className="min-h-screen bg-white dark:bg-[#0a0a0a] pb-10">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        >
          {loading && !termsData.content ? (
            <div className="flex flex-col items-center justify-center py-20">
              <Loader2 className="w-10 h-10 text-[#EF4F5F] animate-spin mb-4" />
              <p className="text-gray-400 font-bold text-xs uppercase tracking-widest animate-pulse">Loading Terms...</p>
            </div>
          ) : termsData.content ? (
            <div
