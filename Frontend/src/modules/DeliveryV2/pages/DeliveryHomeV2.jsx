...
export default function DeliveryHomeV2({ tab = 'feed' }) {
  ...
  const [delayedOnline, setDelayedOnline] = useState(isOnline);
      ...
      <div className={`flex-1 relative overflow-y-auto ${currentTab === 'history' ? 'pt-0' : 'pt-[120px]'} no-scrollbar`}>
         {currentTab === 'feed' ? (
           <div className="absolute inset-0 top-[-120px]">
             <div className={`absolute inset-0 transition-all duration-1000 ease-out ${(delayedOnline || activeOrder) ? 'opacity-100 scale-100 blur-0' : 'opacity-0 scale-95 blur-sm pointer-events-none'}`}>
               <LiveMap 
                 key={delayedOnline ? 'online' : 'offline'} 
